<template>
	<view>	
		...
		<view class="img_box">
			<image :src="showingImg" mode="widthFix" @tap="previewImage"></image>
		</view>
		...
		methods: {
			...
			loadingImg() { //数据请求
				...
				uni.request({
					...
					success: function (res) {
						...
						that.showingImg = that.imgArray[0]
						that.pictureTitle = that.timeArray[0]
						// 						that.imgArray = res.data.d
						// 						var index = that.imgArray[0].indexOf(",") //获得返回字符串“，”的位置
						// 						that.showingImg = that.imgArray[0].substring(0,index)
					}
				})
			},
			previewImage() { //图片预览
			...
			showImage() { //下一张图片
				if (this.imgArray.length > 0) {
					...
					this.pictureTitle = this.timeArray[this.imgindex]
				}
			},
			play() { //播放
				let that = this
